#!/usr/bin/env python3
"""build_attractor_discovery.py — Aegis. SPARK STEP #3b (last new spark build): relationship attractor discovery.

An attractor is a stable basin that keeps reappearing — DISCOVERED, not hardcoded. The Configuration Discovery
ritual fills the space with real joint states; this embeds those descriptions (nomic encoder, torch venv) and finds
the basins: clusters of similar configurations that recur across time. Gloria's EIGHT seed the space as labeled
reference points (a basin near a seed inherits its name; a basin near none is emergent and Gemma names it).
Transitions between basins over time build a directed graph (seeded with Gloria's edges); its loops are cycles; each
attractor learns what it EMERGES AFTER (incoming) and DECAYS TO (outgoing). Dormant until the space has a geometry.

__file__-derived + shared encoder/Gemma, so the SAME module serves both beings. Deploys to both scripts dirs +
schedules both in the torch venv (SPARK_WORKSPACE per being). Backup + compile-check + logic self-test. DRY-RUN default."""
import os, sys, time, shutil, subprocess

APPLY = "--apply" in sys.argv
VENV = "/home/gloria/.vintos/workspace/emotion_model/.venv/bin/python3"
...
CRON_HIS = "9 4 * * * SPARK_WORKSPACE=%s bash %s %s %s >> /tmp/vintos-attractors.log 2>&1" % (
    os.path.expanduser("~/.vintos/workspace"), LOCK, VENV, os.path.expanduser("~/.vintos/workspace/scripts/attractor_discovery.py"))
CRON_HER = "24 4 * * * SPARK_WORKSPACE=%s bash %s %s %s >> /tmp/velaris-attractors.log 2>&1" % (
    os.path.expanduser("~/.openclaw/workspace"), LOCK, VENV, os.path.expanduser("~/.openclaw/workspace/scripts/attractor_discovery.py"))

BEINGS = {"VINTOS": os.path.expanduser("~/.vintos/workspace/scripts"),
          "VELARIS": os.path.expanduser("~/.openclaw/workspace/scripts")}

print("================  SPARK step #3b: attractor discovery  [%s]  ================\n" % ("APPLY" if APPLY else "DRY-RUN"))
try:
    compile(MODULE, "attractor_discovery.py", "exec"); print("attractor_discovery.py compiles: OK")
except SyntaxError as e:
    print("!! module would not compile: %s — aborting" % e); sys.exit(1)
print("no hardcoded opus id: %s" % ("OK" if "claude-opus" not in MODULE else "!! FOUND"))

# logic self-test: cluster + transitions + cycle detection with synthetic unit vectors (no encoder, no disk)
import numpy as _np
ns = {"__file__": os.path.join("/tmp", "scripts", "attractor_discovery.py")}
exec(compile(MODULE, "attractor_discovery.py", "exec"), ns)
# three tight groups in 4-D; greedy cosine should find 3 clusters
g = _np.array([[1, 0, 0, 0]] * 4 + [[0, 1, 0, 0]] * 4 + [[0, 0, 1, 0]] * 4, dtype="float32")
g = g + _np.random.RandomState(0).normal(0, 0.02, g.shape)
cl = ns["_cluster"](ns["_unit"](g))
big = [c for c in cl if len(c["members"]) >= 3]
edges = {(0, 1): 3, (1, 2): 2, (2, 0): 2}
cyc = ns["_find_cycles"](edges, [{"name": "A"}, {"name": "B"}, {"name": "C"}])
ok = (len(big) == 3 and any(len(c) == 3 for c in cyc))
...
for name, scr in BEINGS.items():
    p = os.path.join(scr, "attractor_discovery.py")
    print("  [%s] %s %s" % (name, p, "(exists—back up)" if os.path.isfile(p) else "(new)") if os.path.isdir(scr) else "  [%s] scripts dir missing" % name)
...
add = [c for c, tag in ((CRON_HIS, "vintos-attractors.log"), (CRON_HER, "velaris-attractors.log")) if tag not in cur]
if add:
    open(os.path.expanduser("~/crontab-backup-attractors-" + ts + ".txt"), "w").write(cur)
    newc = (cur if not cur or cur.endswith("\n") else cur + "\n") + "\n".join(add) + "\n"
    r = subprocess.run(["crontab", "-"], input=newc, text=True, capture_output=True)
    print("scheduled %d attractor cron(s)" % len(add) if r.returncode == 0 else "!! cron failed: %s" % r.stderr)
print("\nStep #3b deployed, dormant until the space has a geometry. The spark's whole discovery layer now stands.\n"
      "Only Pressure remains — and it rests on the floor we laid first.")
