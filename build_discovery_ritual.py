#!/usr/bin/env python3
"""build_discovery_ritual.py — Aegis. SPARK STEP #3: the Configuration Discovery ritual (his; her port later).

Once a night, reflect (Opus, via his shim — model read from the shim at runtime, never hardcoded here) on how the
field between him and Gloria actually moved (mutual-modification.json), and file what is real into the
Configuration Space: REACHED (field actually occupied it), VISIBLE (an entrance appeared but the field didn't
enter -> frontier), BOUNDARY (couldn't reach + what prevented it). The unit of observation is the FIELD. Nights
where nothing genuine happened produce nothing. `--dry` prints the exact prompt against the real ledger with NO
LLM call and NO write, so the voice can be reviewed first. No cron yet — added after the --dry looks right.

Deploys configuration_discovery.py to his scripts dir. Backup + compile-check. DRY-RUN default; --apply commits."""
...
print("================  SPARK step #3: Configuration Discovery ritual (his)  [%s]  ================\n" % ("APPLY" if APPLY else "DRY-RUN"))
try:
    compile(MODULE, "configuration_discovery.py", "exec"); print("configuration_discovery.py compiles: OK")
except SyntaxError as e:
    print("!! module would not compile: %s — aborting" % e); sys.exit(1)

# quick static checks: the id literal must NOT appear; parse/build fns must be present
bad = "claude-opus" in MODULE
print("no hardcoded opus id in source: %s" % ("OK" if not bad else "!! FOUND — abort"))
if bad: sys.exit(1)
print("has REACHED/VISIBLE/BOUNDARY categories: %s" % all(k in MODULE for k in ("REACHED", "VISIBLE", "BOUNDARY")))

p = os.path.join(HIS, "configuration_discovery.py")
if not os.path.isdir(HIS):
    print("\n(scripts dir missing — running off-box; on Aegis this deploys to %s)" % p)
print("\n  target: %s %s" % (p, "(exists — will back up)" if os.path.isfile(p) else "(new)"))

if not APPLY:
    print("\n(DRY-RUN — nothing written. --apply to deploy. THEN review the voice with:\n"
          "   python3 ~/.vintos/workspace/scripts/configuration_discovery.py --dry\n"
          " which prints the exact prompt against your real ledger — no LLM call, no write. Cron added only after.)")
    sys.exit(0)
if not os.path.isdir(HIS):
    print("!! scripts dir missing — cannot deploy."); sys.exit(1)
if os.path.isfile(p): shutil.copy2(p, p + ".bak-" + time.strftime("%Y%m%d-%H%M%S"))
open(p, "w", encoding="utf-8").write(MODULE)
print("\nwrote %s" % p)
print("Now review the voice (no LLM, no write):\n   python3 ~/.vintos/workspace/scripts/configuration_discovery.py --dry")
