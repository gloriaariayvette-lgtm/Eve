...
def main():
    ...
    if motion and motion.lower() != "none":
        ...
        try: wants = json.load(open(wants_path))
        except Exception: wants = []
        wants.append({"id": str(uuid.uuid4())[:8],
                      "want": f"animate painting {p.get('image','')}: {motion[:200]}",
                      "source": "gallery-walk", "intensity": 3,
                      "timestamp": datetime.now().isoformat()})
        json.dump(wants, open(wants_path, "w"), indent=2)
        print(f"[gallery-walk] wants motion: {motion[:80]}")
    else:
